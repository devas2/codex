...
async fn chatgpt_auth_registers_agent_identity_when_enabled() -> anyhow::Result<()> {
    ...
    agent_auth.ensure_run_task(Some(server.uri())).await?;
    reused.ensure_run_task(Some(server.uri())).await?;

    assert_eq!(
        agent_auth.record().agent_runtime_id,
        reused.record().agent_runtime_id
    );
    assert_eq!(agent_auth.run_task_id(), Some("task-123".to_string()));
    assert_eq!(reused.run_task_id(), Some("task-123".to_string()));
...
async fn load_auth_reads_access_token_from_env() {
    ...
    assert_eq!(agent_identity.run_task_id(), None);
    assert!(
