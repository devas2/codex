...
async fn responses_lite_strips_http_image_detail_when_resize_all_images_is_enabled() -> Result<()> {
    ...
    .await;
    let image_url = "https://example.com/image.png?signature=unchanged";
    let mut builder = test_codex()
        ...
        .with_config(|config| {
            assert!(config.features.enable(Feature::ResizeAllImages).is_ok());
        });
