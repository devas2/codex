...
#[tokio::test]
async fn auto_image_detail_uses_model_profile() {
    let (_session, mut turn_context) = make_session_and_context().await;

    assert_eq!(
        Session::auto_image_detail(&turn_context),
        AutoImageDetail::High
    );

    turn_context.model_info.auto_image_detail = Some(AutoImageDetail::Original);
    assert_eq!(
        Session::auto_image_detail(&turn_context),
