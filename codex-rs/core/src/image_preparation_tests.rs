...
fn preparation_preserves_small_image_bytes_and_non_data_urls() {
    let (data_url, original_bytes) = png_data_url(64, 32);
    ...
    prepare_response_items(&mut items, AutoImageDetail::High);
...
fn detail_policies_apply_the_expected_budgets() {
    for (detail, auto_detail, input_dimensions, expected_dimensions) in [
        (
            Some(ImageDetail::High),
            AutoImageDetail::Original,
            (2048, 2048),
            (1600, 1600),
        ),
        (
            Some(ImageDetail::Original),
            AutoImageDetail::High,
            (6401, 100),
            (6000, 94),
        ),
        (
        ...
        (None, AutoImageDetail::Original, (2048, 2048), (1600, 1600)),
    ] {
        ...
        prepare_response_items(&mut items, auto_detail);
...
fn preparation_replaces_only_failed_tool_images_and_preserves_metadata() {
    let (valid_image_url, _) = png_data_url(64, 32);
