...
use codex_protocol::openai_models::AutoImageDetail;
...
pub(crate) fn prepare_response_items(items: &mut [ResponseItem], auto_detail: AutoImageDetail) {
    for item in items {
        match item {
            ResponseItem::Message { content, .. } => prepare_message_content(content, auto_detail),
            ResponseItem::FunctionCallOutput { output, .. }
            | ResponseItem::CustomToolCallOutput { output, .. } => {
                if let Some(content) = output.content_items_mut() {
                    prepare_tool_output_content(content, auto_detail);
                }
            }
...
fn prepare_message_content(items: &mut [ContentItem], auto_detail: AutoImageDetail) {
    for item in items {
        if let ContentItem::InputImage { image_url, detail } = item
            && is_data_url(image_url)
            && let Err(error) = prepare_image(image_url, *detail, auto_detail)
        {
...
fn prepare_image(
    ...
    let limits = match detail {
        None | Some(ImageDetail::High) => HIGH_DETAIL_LIMITS,
        Some(ImageDetail::Original) => ORIGINAL_DETAIL_LIMITS,
        Some(ImageDetail::Auto) => match auto_detail {
            AutoImageDetail::High => HIGH_DETAIL_LIMITS,
            AutoImageDetail::Original => ORIGINAL_DETAIL_LIMITS,
        },
