...
fn prompt_with_image_outputs(policy: RequestImageDetailPolicy) -> Prompt {
    Prompt {
        input: vec![
        ...
        image_detail_policy: policy,
        ..Default::default()
    }
}

#[test]
fn request_image_detail_policy_only_strips_responses_lite_copies() {
    let prompt = prompt_with_image_outputs(RequestImageDetailPolicy::StripForResponsesLite);
    ...
    assert_eq!(
        prompt_with_image_outputs(RequestImageDetailPolicy::Preserve)
            .get_formatted_input_for_request(/*use_responses_lite*/ true),
        original
    );
}
