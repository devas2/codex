...
pub enum ProviderAuthScope {
    /// Use the provider's default auth. Agent Identity auth uses its initialized run task here.
    UnscopedProcess,
    /// Resolve first-party provider auth for the current Codex run.
    Run {
...
pub(crate) async fn resolve_provider_auth(
    ...
    {
        agent_identity_auth
            .ensure_run_task(chatgpt_base_url)
            .await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn agent_identity_auth_provider_preserves_account_routing_headers() -> std::io::Result<()>
    {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/v1/agent/agent-runtime-1/task/register"))
